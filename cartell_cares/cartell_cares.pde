...
void setup() {
  frameRate(3);
  size(450, 636, PDF, "cartell_cares_bo9.pdf");
  printArray(fontList);
}
void draw() {
  background(#EBEDE6);
  ...
  float rx3=random(width-150);
  float ry3=random(height-300);
  sergio(int(rx), int(ry));
  julio(int(rx1), int(ry1));
  barbara(int(rx3), int(ry1));
  isa(int(rx1), int(ry));
  alba(int(rx), int(ry3));
  alexis(int(rx2), int(ry2));
  sara(int(rx3), int(ry2));
  laura(int(rx3), int(ry));
  moha(int(rx2), int(ry3));
  titol(20, height-60);
  //exit();
}
...
void barbara(int x, int y) {
  ...
  void titol(int x, int y) {
    ...
    if (i<fontList.length) {
      i++;
      println(i);
      myfont=createFont(fontList[442], 32);
      textFont(myfont);
      fill(0);
      textSize(20);
      text("Classe 2019/2020", 0, 19);
      textSize(15);
      text("Llidóba", 0, 40);
    } 
    popMatrix();
